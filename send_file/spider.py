...
def wait_spider(url, temp_file):
    ...
    while True:
        ...
        if "404 Not Found" in html_string:
            print("SPPIDER: URL not found {}".format(url),
        ...
        if "pr_picture.cgi" in html_string:
            ...
            print("SPPIDER: proccesing {}".format(waiting),
                  file=open(temp_file, "a"))
        time.sleep(5)
        waiting += 5
...
def run(input_params, main_dir,pdb_name):
    ...
    fields = {"PDBFileName": (name, pdb_string, 'text/plan'),
              "Version": "2",
              "Trade": "0.5",
              "Email": "",
              "PDBres": "on"}
    ...
    if not refresh:
        ...
        print("SPPIDER: URL {} refresh".format(temp_url),
              file=open(temp_file, "a"))
        x = refresh[0].attrib['content'].find('http')
        result_url = refresh[0].attrib['content'][x:]
        print("SPPIDER new URL {}".format(result_url),
              file=open(temp_file, "a"))
