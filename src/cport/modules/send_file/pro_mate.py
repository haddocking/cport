...
def wait_promate(url, content, header, temp_file):
    ...
    while True:
        ...
        if "href" in req.text:
            ...
            print(
                "Promate: Send the files again: {}".format(waiting),
...
def run(input_params, main_dir, pdb_name):
    ...
    file = {
        "fileup": (name, pdb_string, "text/plan"),
        "pdbId": "dmmy",
    ...
    if "Failed" in temp_url:
        ...
        return predictors.Predictor(
            pdb=input_params.pdb_file, name="ProMate", success=False
        )
    else:
        ...
        results_pdb = pdb.from_url(
            results_url, name=f"{pdb_name}_ProMate", main_dir=main_dir
        )
        print(
            f"Promate: Finished successfully {results_url}", file=open(temp_file, "a")
        )
