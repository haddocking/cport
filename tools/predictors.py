...
def update_res(predictor_list, surface, distance_list):
    for pred in predictor_list:
        ...
        active_res = []
        # Get the first x residues with the highest score (the x is determined by the threshold)
        if rank is not None:
            active_res = exposed_res[:int(rank)]
        # Get only the residues with score larger than the cutoff (the cutoff is determined by the threshold)
        if score is not None:
            active_res = [i for i in exposed_res if i[1] > float(score)]

        active_res.sort()

        # Get the residues within an acceptable distance (I copied the distance from Cport)
